...
class Netscan:
    ...
    class PortScan:
        ...
        def __init__(self,ports):
            ...
            if self.count == 0:
                try:
                    print(str(Netscan.Port_range[0])+"-"+str(Netscan.Port_range[1])+" -> No ports open")
                except:
                    print(str(Netscan.Port_range[0])+" -> closed")
                finally:
                    print('\n')
    ...
    class NetworkScan:
        def __init__(self):
            print("[*]Please Wait while starting netscan...",end="\r")
        ...
        def printResults(self):
            if self.clients:
                print("Available devices in the network:"+" "*10)
    ...
    class Questions:
        def askQuestions(self):
            ...
            if Netscan.Ipaddr == 'Cancel/Exit Scan':
                ...
                self.GetPortRange(Netscan.Ipaddr)

        def GetPortRange(self,Ipaddr):
            Netscan.Ipaddr = Ipaddr
            ...
            if (0 < len(Netscan.Port_range) <= 2):
                ...
                if len(Netscan.Port_range) == 0:
                    Netscan.Port_range = [1,1000]
                    self.ports = [p for p in range(Netscan.Port_range[0],Netscan.Port_range[1]+1)]
    ...
    def CheckConnection(self):
        ...
        try:
            ...
            if connection:
                Netscan.Questions().askQuestions()
                exit
        except:
